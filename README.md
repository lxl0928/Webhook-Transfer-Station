...
4. 在「中转规则」新增规则，填写 Gitee 仓库页面地址、至少 8 位的回调密钥、事件、提示词、目标机器人地址与消息模板。
5. 保存并复制生成的 `/webhooks?wid=...` URL。wid 的值不需要加引号。
6. 在 Gitee 仓库「管理 → WebHooks」配置此 URL，将相同的回调密钥作为密码，选择 JSON 格式及相应事件。
...
仅在迁移命令成功后执行最后一步。当前迁移 head 为 `0003`；`0002` 增加多源和 @ 配置，`0003` 增加 Skill 与聊天表。降级可能丢数据，不能把迁移回退当作无损撤销。
...
支持 JSON/文本/表单的 HTTP POST 回调、纯文本消息模板和 Chat Completions 兼容调用；不包含公开注册、富文本卡片、附件或模型自动降级。真实模型、仓库和群机器人联调需要自己的凭证。业务日志可能包含仓库内容与个人信息，本版没有自动过期清理，需自行制定备份和保留策略。
